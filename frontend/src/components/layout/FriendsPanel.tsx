...
import { X, UserPlus, Users, Check, Waves } from 'lucide-react'
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        fontSize: size * 0.44, color, lineHeight: 1,
      }}>
        {name[0].toUpperCase()}
      </span>
    </div>
...
export function FriendsPanel() {
  const { friendsOpen, setFriendsOpen, selectedSpotId, userProfile, setMobileTab } = useSpotStore()
  ...
  const sendReq = useMutation({
    ...
    onError: (e: any) => {
      setAddError(e?.response?.data?.detail ?? 'Could not send request')
      setAddSuccess('')
    },
  })
  ...
  function handleSurfNow() {
    ...
    // Use currently selected spot — we need its meta
  ...
  function handleGoToFriend(f: FriendOut) {
    if (!f.surfing) return
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', background: 'rgba(120,184,216,0.08)',
    border: '1px solid rgba(120,184,216,0.25)', borderRadius: 12,
    padding: '10px 14px', color: '#D8EEF8',
          ...
          >
            ...
            <div style={{ padding: '8px 20px 0' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <div>
                  <p style={{ fontFamily: "'Bangers', Impact, system-ui", fontSize: 26, color: '#D8EEF8', lineHeight: 1, letterSpacing: '0.06em', margin: 0 }}>
                    FRIENDS
                  </p>
                  <p style={{ fontFamily: "'Inter', system-ui", fontSize: 12, color: '#5AAAC8', margin: '4px 0 0' }}>
                    See where your crew is surfing
                  </p>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: mySession ? 'rgba(248,113,113,0.15)' : 'linear-gradient(135deg, #78B8D8, #5AAAC8)',
                      border: mySession ? '1px solid rgba(248,113,113,0.40)' : 'none',
                      fontFamily: "'Bangers', Impact, system-ui",
                      fontSize: 12, letterSpacing: '0.12em',
                      color: mySession ? '#F87171' : '#0D1C2A',
                    }}
                  >
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  {surfingNow.map(f => (
                    <div key={f.username} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, background: 'rgba(74,222,128,0.06)', border: '1px solid rgba(74,222,128,0.18)', borderRadius: 14, padding: '10px 12px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <p style={{ fontFamily: "'Bangers', Impact, system-ui", fontSize: 16, color: '#D8EEF8', margin: 0, letterSpacing: '0.04em' }}>{f.username}</p>
                        <p style={{ fontFamily: "'Inter', system-ui", fontSize: 11, color: '#4ADE80', margin: '2px 0 0' }}>● {f.surfing!.spot_name}</p>
                      </div>
                      <button
                        onClick={() => handleGoToFriend(f)}
                        style={{ padding: '5px 10px', borderRadius: 8, background: 'rgba(74,222,128,0.15)', border: '1px solid rgba(74,222,128,0.30)', fontFamily: "'Bangers', Impact, system-ui", fontSize: 10, letterSpacing: '0.10em', color: '#4ADE80', cursor: 'pointer' }}
                      >MAP →</button>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                {(['friends', 'add'] as Tab[]).map(t => (
                  <button key={t} onClick={() => setTab(t)} style={{ padding: '6px 14px', borderRadius: 10, cursor: 'pointer', fontFamily: "'Bangers', Impact, system-ui", fontSize: 12, letterSpacing: '0.12em', background: tab === t ? 'rgba(120,184,216,0.18)' : 'rgba(120,184,216,0.07)', border: `1px solid ${tab === t ? 'rgba(120,184,216,0.40)' : 'rgba(120,184,216,0.14)'}`, color: tab === t ? '#78B8D8' : '#5AAAC8' }}>
                    {t === 'friends' ? `FRIENDS (${accepted.length})` : 'ADD FRIEND'}
                  </button>
                ))}
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 8 }}>
                  ...
                  {pendingIn.map(f => (
                    <div key={f.username} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.22)', borderRadius: 14, padding: '10px 12px' }}>
                      ...
                      <button onClick={() => acceptReq.mutate(f.username)} style={{ padding: '6px 10px', borderRadius: 8, background: 'rgba(74,222,128,0.15)', border: '1px solid rgba(74,222,128,0.30)', color: '#4ADE80', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                        <Check size={14} />
                      </button>
                      <button onClick={() => removeReq.mutate(f.username)} style={{ padding: '6px 10px', borderRadius: 8, background: 'rgba(248,113,113,0.10)', border: '1px solid rgba(248,113,113,0.25)', color: '#F87171', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                        <X size={14} />
                      </button>
                    </div>
                  ))}
                  ...
                  {pendingSent.map(f => (
                    <div key={f.username} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(13,28,42,0.60)', border: '1px solid rgba(120,184,216,0.10)', borderRadius: 14, padding: '10px 12px', opacity: 0.7 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <p style={{ fontFamily: "'Bangers', Impact, system-ui", fontSize: 16, color: '#D8EEF8', margin: 0 }}>{f.username}</p>
                        <p style={{ fontFamily: "'Inter', system-ui", fontSize: 11, color: '#5AAAC8', margin: '2px 0 0' }}>Request sent…</p>
                      </div>
                  ...
                  {accepted.length === 0 && pendingIn.length === 0 && pendingSent.length === 0 && (
                    <p style={{ fontFamily: "'Inter', system-ui", fontSize: 13, color: '#3A6A8A', textAlign: 'center', padding: '24px 0' }}>No friends yet — add some from the ADD FRIEND tab</p>
                  )}
                </div>
              )}
              ...
              {tab === 'add' && (
                <div style={{ paddingBottom: 8 }}>
                  <p style={{ fontFamily: "'Inter', system-ui", fontSize: 13, color: '#5AAAC8', marginBottom: 10 }}>Search by username</p>
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button
                      ...
                      style={{ padding: '10px 16px', borderRadius: 12, background: 'linear-gradient(135deg, #78B8D8, #5AAAC8)', border: 'none', cursor: 'pointer', fontFamily: "'Bangers', Impact, system-ui", fontSize: 13, letterSpacing: '0.12em', color: '#0D1C2A', flexShrink: 0 }}
                    >
                      <UserPlus size={16} />
                    </button>
                  </div>
                  {addError && <p style={{ fontFamily: "'Inter', system-ui", fontSize: 12, color: '#F87171', marginTop: 8 }}>{addError}</p>}
                  {addSuccess && <p style={{ fontFamily: "'Inter', system-ui", fontSize: 12, color: '#4ADE80', marginTop: 8 }}>{addSuccess}</p>}
                </div>
              )}
            </div>
          </motion.div>
